...
def validate_directory(location):
    ...
    1]  Path exists
    2]  It's a directory/path
    3] It contains at least one .xlsx file inside it
    """
    validation_message = ""
    # Check if the location exists
    if not os.path.exists(location):
        validation_message = "Invalid path/location, please check"
        return False, validation_message

    # Check if the location is a directory
    if not os.path.isdir(location):
        validation_message = "Passed path/location is not a directory/folder"
        return False, validation_message

    # Check if there is at least one .xlsx file in the directory
    for file in os.listdir(location):
        if file.endswith('.xlsx'):
            return True, validation_message

    validation_message = "Passed directory/folder does not contain any excel sheet"
    return False, validation_message


def validate_excel(folder_path, file_name):
    validation_message = ""
    ...
    if not os.path.exists(location):
        validation_message = "File does not exist."
        return False, validation_message
    ...
    column_level_df = pd.read_excel(location, sheet_name='ColumnLevel', dtype=str)
    table_level_df = pd.read_excel(location, sheet_name='TableLevel', dtype=str)
...
def list_all_valid_excels(location):
    """
...
def process_excel_file(directory_path, excel_file_name):
    ...
    # Read the Excel file
    file_path = f"{directory_path}/{excel_file_name}"
    column_level_df = pd.read_excel(file_path, sheet_name='ColumnLevel', dtype=str).fillna('')
    table_level_df = pd.read_excel(file_path, sheet_name='TableLevel', dtype=str).fillna('')
...
def write_yaml_files(yaml_content_dict, output_directory):
    ...
    Args:
        ...
        output_directory (str): Directory to save the YAML files.
    """
    for key, value in yaml_content_dict.items():
        ...
        try:
            with open(yaml_file_path, 'w') as yaml_file:
                yaml.dump(yaml_content, yaml_file, default_flow_style=False, sort_keys=False, Dumper=yaml.SafeDumper)
        except IOError as e:
...
def main():
    ...
    location = input("Please enter complete folder path where you placed DQ excel sheets--> ")
